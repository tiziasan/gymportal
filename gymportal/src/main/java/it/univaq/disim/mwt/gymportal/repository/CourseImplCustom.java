...
public interface CourseImplCustom {
		...
		public Set<Course> findCourseByGymId(long gym_id);
		...
		public Set<Course> searchByIdAndName(long id, String name);
		
		@Query(value = "FROM Course AS c WHERE c.gym.id = :id AND c.gym.user.id = :idUser AND c.name LIKE CONCAT('%',:name,'%')")
		public Set<Course> searchByIdAndNameAndUser(long id, String name, long idUser);
